...
function TenantRegisterPageContent() {
  ...
  const loginLink = redirectUrl 
    ? `/auth/login/tenant?redirect=${encodeURIComponent(redirectUrl)}`
    : "/auth/login/tenant";

  if (isRegistered) {
    return (
      <>
        <div className="flex-grow flex items-center justify-center py-16 px-5 bg-gradient-to-br from-[#fdfcfb] via-background to-[#fef6ee]">
          <div className="w-full max-w-lg bg-white border border-[#f07d00]/25 p-10 rounded-2xl shadow-xl text-center">
            <div className="w-20 h-20 bg-[#f07d00]/10 border border-[#f07d00]/20 rounded-full flex items-center justify-center mx-auto mb-8 animate-pulse">
              <span className="material-symbols-outlined text-[48px] text-[#f07d00]" style={{ fontVariationSettings: "'FILL' 1" }}>
                mail
              </span>
            </div>
  ...
  return (
    <>
      <div className="flex-grow flex items-center justify-center py-16 px-5 bg-gradient-to-br from-[#fdfcfb] via-background to-[#fef6ee]">
        <div className="w-full max-w-lg bg-white border border-[#f07d00]/20 p-8 rounded-3xl shadow-xl relative overflow-hidden">
          {/* Trademark Heimstadt Orange line */}
          <div className="absolute top-0 left-0 w-full h-[6px] bg-[#f07d00]" />
          
          <div className="text-center mb-8">
            <div className="w-14 h-14 bg-[#f07d00]/10 text-[#f07d00] border border-[#f07d00]/20 rounded-2xl flex items-center justify-center mx-auto mb-4">
              <span className="material-symbols-outlined text-[32px]">person_add</span>
            </div>
            <span className="inline-block px-3 py-1 rounded-full text-[10px] font-extrabold bg-[#f07d00]/5 text-[#f07d00] uppercase tracking-wider mb-2">
              {language === "de" ? "Mieter Portal" : "Tenant Portal"}
            </span>
            <h1 className="text-[24px] md:text-[26px] text-primary font-black tracking-tight leading-none mb-2">
              {t("registerTenantTitle")}
            </h1>
            <p className="text-on-surface-variant text-[14px]">
              {t("registerTenantSubtitle")}
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-4">
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  className="w-full h-11 px-4 bg-surface border border-outline-variant rounded-xl focus:ring-2 focus:ring-primary focus:border-transparent outline-none transition-all text-[15px]"
                />
              </div>
            ...
            <button
              ...
              className="w-full h-11 bg-primary text-on-primary rounded-xl font-bold hover:opacity-90 active:scale-98 transition-all shadow-md mt-6 cursor-pointer disabled:opacity-50 flex items-center justify-center gap-2 text-[14px]"
            >
              {loadingSubmit && (
          ...
          <div className="mt-6 text-center text-[13px]">
            ...
            <div className="mt-2 text-[11px] text-on-surface-variant/80">
              {language === "de" ? "Sie sind Vermieter?" : "Are you a landlord?"}{" "}
              <Link href="/auth/register/landlord" className="text-primary font-bold hover:underline">
...
export default function TenantRegisterPage() {
  return (
    <Suspense fallback={
      <div className="flex-grow flex items-center justify-center min-h-[600px] bg-[#fffcf9]">
        <div className="relative w-16 h-16 flex items-center justify-center">
          <div className="absolute inset-0 rounded-full border-[3px] border-[#f07d00]/15 border-t-[#f07d00] animate-spin" />
          <div className="absolute w-10 h-10 rounded-full border-[3px] border-[#ffebd6]/20 border-b-[#ffebd6] animate-spin [animation-direction:reverse] [animation-duration:1s]" />
        </div>
      </div>
    }>
