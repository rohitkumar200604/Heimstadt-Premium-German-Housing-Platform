...
function TenantLoginPageContent() {
  ...
  useEffect(() => {
    if (profile) {
      if (!profile.role) {
        ...
        const destination = redirectUrl || "/dashboard/tenant";
      ...
      if (profileData) {
        if (!profileData.role) {
          try {
            ...
            sessionStorage.removeItem("auth_role");
            router.push(redirectUrl || "/dashboard/tenant");
            return;
          } catch (err) {
            console.error("Auto assigning role failed:", err);
          }
  ...
  const registerLink = redirectUrl 
    ? `/auth/register/tenant?redirect=${encodeURIComponent(redirectUrl)}`
  ...
  return (
    <>
      <div className="flex-grow flex items-center justify-center py-16 px-5 bg-gradient-to-br from-[#fdfcfb] via-background to-[#fef6ee]">
        <div className="w-full max-w-md bg-white border border-[#f07d00]/20 p-8 rounded-3xl shadow-xl relative overflow-hidden">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="space-y-1.5">
              ...
              <input
                ...
                type="email"
                required
                placeholder="beispiel@heimat.de"
                value={email}
            ...
            <button
              ...
              className="w-full h-11 bg-primary text-on-primary rounded-xl font-bold hover:opacity-90 active:scale-98 transition-all shadow-md cursor-pointer disabled:opacity-50 flex items-center justify-center gap-2 text-[14px]"
            >
              {loadingSubmit && (
          ...
          <div className="mt-6 text-center text-[13px]">
            ...
            <div className="mt-2 text-[11px] text-on-surface-variant/80">
              ...
              <Link href="/auth/login/landlord" className="text-primary font-bold hover:underline">
                {language === "de" ? "Zum Vermieter-Login" : "Go to Landlord Login"}
              </Link>
            </div>
          </div>
        </div>
